...
export default function KnowldegeAboutFosterItem({
   data,
}: IKnowldegeAboutFosterItemProps) {
   return (
      <li className='w-full h-[33.33333%] flex flex-col md:flex-row items-center gap-3 md:gap-6'>
         ...
         <div className='flex-1 py-4 flex flex-col justify-between gap-1'>
            <Link
               href={'/'}
               className='text-2xl text-green-5FA503 font-semibold uppercase hover:underline'
            >
               {data.title}
            </Link>
            <p className='text-black-main text-1xl line-clamp-5'>
               {data.contents}
            </p>
         </div>
      </li>
